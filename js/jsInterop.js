...
window.D3TimeMapUtilities = {
    ...
    createD3Tree: function (jsonTimeMapData) {
            ...
            .on("touchend", function () {
                if (isTap) {
                    ...
                    if (d3.event.changedTouches[0].target.id.includes("proximityrect")) {
                        ...
                        parentnodeid = invokeDotNetMethod(window.mapHasChanged, "GetParentNodeIdFromProximalNode", proximalnodeid, d3.mouse(document.getElementsByTagName("g")[0]));
                    }
                ...
                if (!flexdivdragging) {
                    if (d3.event.target.nodeName !== "PATH") {
                        if (!(d3.event.transform.k === 1 &&
                            ...
                            invokeDotNetMethod(true, "UpdateTranslation", d3.event.transform);
                ...
                .on("drag", function (d) {
                    ...
                    if (dragWithCtlKey) {
                        if (d.data.parentNodeId !== 0) {
                            let nodeLocation = JSON.parse(invokeDotNetMethod(true, "UpdateImpactedNode", d.data.nodeid, d.x, d.y, true));
                    ...
                    if (dragWithCtlKey) {
                        ...
                        d.descendants().forEach((descendant) => {
                            ...
                            if (d.data.nodeid != descendant.data.nodeid) {
                                ...
                                let nodeLocation = JSON.parse(invokeDotNetMethod(true, "UpdateImpactedNode", descendant.data.nodeid, descendant.x, descendant.y, true));
        ...
        function recalcNodeLinks(d) {
            ...
            nodeLocations.forEach((locationstring) => {
        ...
        function singleNodeDragFinalise(d) {
            ...
            childpaths.attr("d", p => {
                let nodeLocation = JSON.parse(invokeDotNetMethod(true, "UpdateImpactedNode",
                        ...
                        function (enter) {
                            ...
                            menupar.append(menupar.attr("data-menuelement"))
                                ...
                                .on("input", function (d) {
                                    d3.event.stopPropagation();
                                    if (d.value.menutype === "checkbox") {
                                        invokeDotNetMethod(true, d.value.dotnetmethod, parseInt(this.dataset.nodeid), this.checked);
                                        ...
                                        invokeDotNetMenuMethod(d.value.dotnetmethod, nodestochange, this.value);
                                        if (d.value.menutype === "color") {
                                            updatenodecolour(nodestochange, this.value);
                                        }
                                        else if (d.value.attributename === "fontsizepx") {
        ...
        function switchParent(childid, oldparentid, newparentid) {
            ...
            if (updatednode.data.parentNodeId === newparentid) {
                ...
                const changedNodes = d3.set(invokeDotNetMethod(window.mapHasChanged, "GetChangedNodeSet").result);
        ...
        function d3updateReplaceChild(childid, oldparentid, newparentid) {
            ...
            updateLinks(getChangedLinks(d3.set().add(newparentid).add(childid).add(oldparentid)),
                d3.set().add(newparentid).add(childid).add(oldparentid));
        ...
        function d3updateNodesArray(nodesArray) {
            nodesToUpdate = root.descendants().filter(node => d3.set(nodesArray).has(node.data.nodeid))
            nodeUpdate(nodesToUpdate, d3.set(nodesArray));
            const nodeSetWithParents = addParentNodesToChangedNodeSet(d3.set(nodesArray));
            updateLinks(getChangedLinks(nodeSetWithParents), nodeSetWithParents);
            updateProximityRects(nodesToUpdate, d3.set(nodesArray));
        }

        function d3updateAllNodes() {
            const changedNodes = d3.set();
